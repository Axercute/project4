import { e as ensure_array_like, b as attr_class, a as attr, c as bind_props } from "../../../chunks/index2.js";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let displayMenu = [];
    const filterMenu = (selectedChoice2) => {
      displayMenu = contactUsInfo.filter((element) => element.branch === selectedChoice2);
    };
    ...
    $$renderer2.push(`<!--]--></div> <div class="bg-gradient-to-br from-[#7d1b1f] to-red-700 p-2 flex-center flex-col w-[75%] mx-auto mt-50 rounded-2xl outline-2 outline-black shadow-2xl shadow-cyan-800 md:w-2/3 xl:w-1/2"><div class="flex font-semibold text-white text-2xl text-center m-2">${escape_html(_w_runtime_.t(34))}</div> <div class="flex font-semibold text-white text-center md:text-md m-2">${escape_html(_w_runtime_.t(35))}</div> <!--[-->`);
    const each_array_1 = ensure_array_like(displayMenu);
    for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
      let element = each_array_1[$$index_1];
      $$renderer2.push(`<div class="flex font-semibold text-white text-center m-2">${escape_html(element.address)}</div> <a${attr("href", `https://wa.me/${element.phone}?text=I would like to ask if you have...`)} target="_blank" class="bg-white rounded-md hover:bg-green-400 p-2 m-2">${escape_html(_w_runtime_.t(36))}</a> <a${attr("href", element.googleMap)} target="_blank" class="bg-white mt-1 rounded-md hover:bg-green-400 p-2 m-2">${escape_html(_w_runtime_.t(37))}</a>`);
    }
    $$renderer2.push(`<!--]--></div>`);
    bind_props($$props, { contactUsInfo });
  });
}
export {
  _page as default
