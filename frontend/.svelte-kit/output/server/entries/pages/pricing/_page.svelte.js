...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (!treatments) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="lds-dual-ring"></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
      ...
      for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
        ...
        $$renderer2.push(`<div class="bg-red-800 text-white text-md font-semibold rounded-xl p-4 shadow"><div class="mb-1 text-lg font-bold">${escape_html(element.english_name)}</div> <div class="mb-2 text-sm">${escape_html(element.description)}</div> <div class="text-right text-md font-bold">$${escape_html(element.starting_price)}</div></div>`);
      }
